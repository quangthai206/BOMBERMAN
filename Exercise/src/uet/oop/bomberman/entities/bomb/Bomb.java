...
public class Bomb extends AnimatedEntitiy {
	...
	private BombMusic bombMusic = new BombMusic();
	...
	public Bomb(int x, int y, Board board, boolean check) {
		_x = x;
		_y = y;
		_board = board;
		_sprite = Sprite.bomb;
	...
	protected void explode() {
		...
		if(a != null)  {
			a.kill();
                ...
                if(check) r = Game.getBombRadius2();
		
		_flames = new Flame[4];
		
		for (int i = 0; i < _flames.length; i++) {
			_flames[i] = new Flame((int)_x, (int)_y, i, r, _board);
		}
	}
